var Game = function(){
      ...
      draw: function(ctx){
        ...
        for(i in myBoxes){
          if(myBoxes.hasOwnProperty(i) && myBoxes[i] ){
              ...
              ctx.globalAlpha = 0.7;
              ctx.strokeRect(w*boxWidth+w+2, h*boxHeight+h+2, boxWidth-3, boxHeight-3);
          }
        }
        ctx.restore();
      },
  ...
  var getShape = function(){
    ...
    switch(Math.floor(Math.random()*7)){
      case 0: 
        shape = new Shape([1, 1, 1, 1], 'maroon');
        ...
        shape.offsetX = 1;
        return shape;
      case 1: return new Shape([0, 0, 0, 1, 1, 1, 0, 0, 1], 'white');
      case 2: return new Shape([0, 0, 0, 0, 0, 1, 1, 1, 1], 'magenta');
      case 3:
        shape = new Shape([0, 0, 0, 0, 1, 1, 0, 1, 1], 'dark blue');
        shape.rotate = function(){};
        return shape;
      case 4: return new Shape([0, 0, 0, 0, 1, 1, 1, 1, 0], 'green');
      case 5: return new Shape([0, 0, 0, 0, 1, 0, 1, 1, 1], 'brown');
      case 6: return new Shape([0, 0, 0, 1, 1, 0, 0, 1, 1], 'cyan');
    }
  };
  ...
  var redraw = function(){ 
    ...
    if(running){
      ...
      if(paused){
        ...
        ctx.fillText('Press \'p\' to unpause', 10, 150);
      }
    }else{
...
// magic
  ...
  ctx.fillStyle = 'rgba(0,0,0,0.8)';
  ctx.font         = 'bold 24px sans-serif';
  ctx.fillText('Press \'s\' to play', 10, 150);
  pointsContainer.appendChild(document.createElement('br'));
  pointsContainer.setAttribute('id', 'points');
  sidepanel.setAttribute('id', 'sidepanel');
  bottom.setAttribute('id', 'bottom');
  sidepanel.appendChild(document.createTextNode('Next shape: '));
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(nextCanvas);
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(pointsContainer);
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(document.createTextNode('s to start'));
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(document.createTextNode('← to move tile left'));
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(document.createTextNode('→ to move tile right'));
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(document.createTextNode('↑ to turn current tile'));
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(document.createTextNode('↓ to increase falling speed'));
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(document.createTextNode('r to restart'));
  sidepanel.appendChild(document.createElement('br'));
  sidepanel.appendChild(document.createTextNode('p to pause or unpause'));
  sidepanel.appendChild(document.createElement('br'));
  bottom.appendChild(document.createTextNode('The amount of points earned increases when multiple rows are removed at once. One row generates one points, but four rows generates a total of ten points.'));
